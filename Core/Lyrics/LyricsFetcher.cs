...
namespace OpenMediaBridge.Lyrics.Fetchers
{
    ...
    public class LyricsFetcher
    {
        ...
        public List<LyricsLine> GetCurrentRawLines() => GetCurrentLines();
        ...
        public string GetCurrentLineWordSync(long positionMs)
        {
            ...
            if (idx >= _lyrics.Count - 1)
            {
                // Last line — no next line to compute interval from; use a 5-second window
                interval = 5000;
            }
            else
            {
            ...
            if (elapsed >= (long)totalWithPauses)
                // All words highlighted — show plain text (no yellow) so the line
                // stays visible until the next line starts, rather than going blank.
                return current.Text ?? "";

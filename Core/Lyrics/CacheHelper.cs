...
namespace OpenMediaBridge.Lyrics.Fetchers
{
    public static class CacheHelper
    {
        ...
        public static void ClearCache(string title, string artist)
        {
            try
            {
                var path = GetCachePath(LyricsFetcher.CacheFolder, artist, title);
                if (File.Exists(path))
                {
                    File.Delete(path);
                }
        ...
        public static string GetTranslatedCachePath(string cacheFolder, string artist, string title, string lang)
        {
            Directory.CreateDirectory(cacheFolder);
            var readable = $"{Sanitize(artist)}-{Sanitize(title)}";
            var hash = ComputeHash(artist + "|" + title);
            return Path.Combine(cacheFolder, $"{readable}-{hash}.{lang}.json");
        }

        public static bool TryLoadTranslated(string cacheFolder, string artist, string title, string lang, out List<LyricsLine> lines)
        {
            lines = null;
            try
            {
                var path = GetTranslatedCachePath(cacheFolder, artist, title, lang);
                if (!File.Exists(path)) return false;
                var json = File.ReadAllText(path);
                var payload = JsonSerializer.Deserialize<CachePayload>(json);
                lines = payload?.Lines ?? new List<LyricsLine>();
        ...
        public static void SaveTranslated(string cacheFolder, string artist, string title, string lang, List<LyricsLine> lines)
        {
            try
            {
                var path = GetTranslatedCachePath(cacheFolder, artist, title, lang);
